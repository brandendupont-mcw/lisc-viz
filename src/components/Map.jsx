...
mapboxgl.accessToken =
  "pk.eyJ1IjoiYnJhbmRlbi1kdXBvbnQiLCJhIjoiY2lmeWZqcXVhNTJwdHUzbTJwbGx2NjljNSJ9.fbeofSfEbOnewOSrZ-wMqA";
...
export default class Map extends React.Component {
  constructor(props) {
    ...
    this.state = {
      lat: 43.0370209990098,
      lng: -87.93627256537013,
      zoom: 11.7,
    };
  }
  ...
  componentDidMount() {
    ...
    this.map = new mapboxgl.Map({
      ...
      style: "mapbox://styles/branden-dupont/ckhsonta12lbb19koc7t5e9t9",
      center: [lng, lat],
      zoom,
      minZoom: 9,
      maxZoom: 25,
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (this.map.isStyleLoaded()) {
      if (nextProps.chapterName !== chapterName) {
        ...
        // controls geojson on top of vector files?
        layout.forEach((data) => {
          this.map.setLayoutProperty(data.id, "visibility", data.visibility);
        });

        // this function is what controls the smooth scroll transition between maps
        //
      ...
      if (nextProps.amenity) {
        if (nextProps.amenity !== amenity) {
          if (nextProps.amenity !== "all") {
            this.map.setFilter("evacuation", [
              "==",
              "amenity",
              nextProps.amenity,
            ]);
            this.map.setFilter("capacity", [
              "==",
              "amenity",
              nextProps.amenity,
            ]);
            this.map.setFilter("radius", ["==", "amenity", nextProps.amenity]);
            this.map.setFilter("walking", [
              "all",
      ...
      if (nextProps.buildingType) {
        if (nextProps.buildingType !== buildingType) {
          if (nextProps.buildingType !== "all") {
            this.map.setFilter("buildings", [
              "==",
              "category",
              nextProps.buildingType,
            ]);
          } else {
            this.map.setFilter("buildings", undefined);
          }
